...
route: /infra-templates
---

## Introduction

As TradeTrust is an open ecosystem, we have developed a template that can be used to quickly deploy your own infrastructure.
...
These templates are Open Source have been written using the [Serverless Framework](https://www.serverless.com/framework/docs/).
...
To use the domain manager functionality you need to first set it up:

```bash
npm run create-domain:verify
```

If you do not wish to use the domain manager functionality, you can configure it like this:
