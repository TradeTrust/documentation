...
If you have the wallet you can convert the wallet into a string like the above example, please use this [online tool](https://tools.knowledgewalls.com/jsontostring).

Alternatively, you can refer to [Open Attestation's documenation](https://github.com/Open-Attestation/open-attestation-cli#wallet) to create your wallet.(Note: you will need to have Open Attestation Cli installed)
...
- Note: This document storage field is optional.

##### If you do not want to upload your documents to a document storage enpoint you can omit this in the config file.

To add a document storage endpoint, you will have to have the end point set up.
...
If your document storage endpoint does not requires any api key, you omit this field.

### `"url"` field

The `"url"` field accepts a string which will be the endpoint of the document storage.

- Note: please ensure that this document storage endpoint has a path `"/storage/queue"` that will return the queue number for storing your document in the document storage endpoint.

Alternatively, we have a default document storage endpoint that has been set up.

The default document storage endpoint currently is implemented for both ropsten testnet and rinkeby testnet only.
