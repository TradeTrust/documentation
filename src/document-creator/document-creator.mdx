...
If you don't have a wallet, you can refer to [Open Attestation's documentation](https://github.com/Open-Attestation/open-attestation-cli#wallet) to create a wallet.
...
Alternatively, you can also generate a wallet.json from an existing account created in Metamask using the OpenAttestation Cli. _Note: you will need to have Open Attestation Cli installed._ 
...
Once you have your wallet.json, the only thing you need to do is to run JSON.stringify over the contents in order to serialise it into a string.
...
```
This result can now be entered into the `"wallet"` field in the [config file](/document-creator#configuration-file).
