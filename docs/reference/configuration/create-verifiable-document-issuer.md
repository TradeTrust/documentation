...
You can read more about the components [here](/docs/tutorial/verifiable-documents/ethereum/document-store-overview).
...
4. Edit Document Form.
   ...
   - `wrapDocument` and `issueDocument` is called on click of the `Submit` button. Thereafter, `wrappedDocument` is set within the application state at this point.
     - The `$template` field specifies the template to be used to render the verifiable document. You can learn how to create your own document renderer [here](docs/reference/configuration/create-custom-renderer).
