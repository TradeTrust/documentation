...
TradeTrust ensures that the content of the document has not been modified since the document has been created, with exception of data removed using the built-in [obfuscation mechanism](#selective-redaction). Let's explore how it works.

In the tutorial section, we have learnt how to [wrap a document](/docs/tutorial/transferable-records/wrapping-document) and [issue it](/docs/tutorial/transferable-records/issuing-document-code) into a document store. However, we didn't explain what these actions were doing and why they are necessary.
