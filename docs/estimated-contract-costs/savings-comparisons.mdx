...
title: Token Registry V4 Cost Savings Comparisons
---
...
Are you a new user of TradeTrust or looking at deploying a new Token Registry (Transferable Documents)? If you are, we'd totally suggest that you try out our new V4 version!

Not only will you be able to try out the new features in our new V4 version, you will also enjoy massive cost savings when deploying your contracts!
...
The new V4 contracts can help to save up to about 92% in total gas fees compared to its previous versions.
If the costs on Ethereum is still a concern despite the savings, you can consider deploying onto the Polygon network, which the V4 repository comes preconfigured with.
The estimations on this page are based on the current prices on the Ethereum and Polygon mainnet.

Now, if you are convinced to try our new V4 version, check out our repository [here](https://github.com/Open-Attestation/token-registry/tree/master)! 🎉
      ...
      <tr key={idx}>
        ...
        <td align="center">
          ...
          {!v3Gas ? null : savings <= 0 ? (
            <Triangle color="#22c55e" style={{ marginLeft: "8px", display: "inline", verticalAlign: "baseline" }} />
          ) : (
            <Triangle
              color="#f43f5e"
              style={{ marginLeft: "8px", transform: "rotate(180deg)", display: "inline", verticalAlign: "baseline" }}
            />
          )}
        </td>
