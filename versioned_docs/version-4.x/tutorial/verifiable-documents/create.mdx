...
While there exists many [DIDs](/docs/4.x/topics/appendix/glossary#did), this tutorial will focus only on `ethr` DID. At the moment, TradeTrust only supports `ethr` DID.
...
- Using the [wallet created in the prerequisite step](/docs/4.x/tutorial/prerequisites#wallet-creation), we run this command on the CLI to retrieve the private key:
