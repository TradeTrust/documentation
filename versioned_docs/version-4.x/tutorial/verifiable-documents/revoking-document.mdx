...
- some kind of DID documents:
  1. base DID document (`ISSUER_IDENTITY`: `identityProof.identifier` = [`ethr` DID](/docs/4.x/topics/introduction/issuer-method-dns-did))
  2. DNS-DID document (`ISSUER_IDENTITY`: `identityProof.identifier` = [DNS-DID](/docs/4.x/topics/introduction/issuer-method-dns-did))
- a deployed `documentStore`, click [here](/docs/4.x/tutorial/verifiable-documents/advanced/document-store/deploying-document-store/document-store-cli) for how to do so.
...
- `--hash` is the value of `targetHash` field of one of the [previously wrapped documents](/docs/4.x/tutorial/verifiable-documents/wrapping-document/wrapping-document-cli) (open one of the file, head to the bottom and check for the `targetHash` in the `signature` object.)
...
If you open the [previously wrapped documents](/docs/4.x/tutorial/verifiable-documents/wrapping-document/wrapping-document-cli) side by side, you will notice:
